...
string get_current_clock_string() {
	...
	stringstream ss;
	ss << put_time(localtime(&in_time_t), "%H-%M-%S") << "-" << setfill('0') << setw(3) << ms.count();  // append milliseconds
...
void CreateLog()  // god thank chatgpt
{
	...
	// Read old latest log content
	ifstream oldLog(mainLog);
	string line;
	vector<string> lines;
